      ...
      if (s != "") {
		...
		for (int i = 0; i < sensor_fusion.size(); i++)
		{
		  ...
		  if(d < (2+4*lane+2) && d > (2+4*lane-2) )
		  {
		    ...
		    if((check_car_s > car_s) && ((check_car_s - car_s) <30))//I forget what this extra bit was doing: && (fmod(round(car_d),2.0)==0))
		    {
		      ...
		      too_close_speed = check_speed;
                      too_close_sd = check_car_s - car_s;
...
//Adding basic logic for lane changes
		      ...
		      closest_rear_Rv = 100000.0;
                      ...
                      for (int i2 = 0; i2 < sensor_fusion.size(); i2++)
                      {
                        ...
                        if ((d2 < (2+4*(lane-1)+2)) && (d2 >(2+4*(lane-1)-2)))
                        {
			  ...
			  distance = abs(check_car_s2 - car_s);
			  if(car_s < check_car_s2)
			  {
			  ...
			  if(car_s < check_car_s2)
			  {
				...
				//cout<<"closest front right: "<<closest_front_R;
			    }
			  }
			  else
			  {
		...
		if (closest_front_R < 30){costR += 1000;}
		if (closest_rear_R < 30){costR += 1000;}
		double costL = 0.0;	
		...
		//if (closest_front_Lv!=100000.0){costL += (1/closest_front_Lv)*5;}
		if (closest_front_L < 30){costL += 1000;}
		if (closest_rear_L < 30){costL += 1000;}


		if(too_close){
	    	  //slow down
		  ref_vel -= .056;
		  if(too_close_sd < 10){ref_vel -= .224;}
